...
class BaseEnv(gym.Env):
    ...
    SUPPORTED_REWARD_MODES = ("delta", "normalized_dense", "dense", "sparse", "none")
    ...
    def get_reward(self, obs: Any, action: torch.Tensor, info: Dict):
        ...
        elif self._reward_mode == "delta":
            reward = self.compute_delta_reward(obs=obs, action=action, info=info)
    ...
    def compute_delta_reward(self, obs: Any, action: torch.Tensor, info: Dict):
        normalized = self.compute_normalized_dense_reward(obs=obs, action=action, info=info)
        diff = normalized - self.last_reward
        self.last_reward = normalized
        return 10*diff
    ...
    def reset(self, seed: Union[None, int, list[int]] = None, options: Union[None, dict] = None):
        ...
        if self._reward_mode == "delta":
            # TODO: 1. Why does this need action? 2. Is there a way to avoid computing on the entire batch with how info is passed?
            self.last_reward[env_idx] = self.compute_normalized_dense_reward(obs=obs, action=None, info=info)[env_idx]
